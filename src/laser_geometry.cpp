...
void
                                   ...
                                   bool preservative, int mask)
  {
    ...
    if ((mask & laser_geometry::channel_option::Intensity) && scan_in.intensities.size() > 0)
    {
    ...
    if (mask & laser_geometry::channel_option::Index)
    {
      int chan_size = cloud_out.channels.size();
      cloud_out.channels.resize (chan_size +1);
    ...
    if (mask & laser_geometry::channel_option::Distance)
    {
      int chan_size = cloud_out.channels.size();
      cloud_out.channels.resize (chan_size + 1);
    ...
    if (mask & laser_geometry::channel_option::Timestamp)
    {
      int chan_size = cloud_out.channels.size();
      cloud_out.channels.resize (chan_size + 1);
...
const boost::numeric::ublas::matrix<double>& LaserProjection::getUnitVectors_(double angle_min, double angle_max, double angle_increment, unsigned int length)
                                                     ...
                                                     tf::Transformer& tf, double range_cutoff, int mask)
  {
    ...
    mask |= laser_geometry::channel_option::Index;
    ...
    for(unsigned int i = 0; i < cloud_out.points.size(); ++i)
    {
      ...
      tfScalar ratio = rotation_direction_?pt_index / ( (double) scan_in.ranges.size() - 1.0):1.0-pt_index / ( (double) scan_in.ranges.size() - 1.0) ;
      ...
      cur_transform.setRotation( tf::slerp( q1, q2 , ratio) ) ;
                                      ...
                                      int channel_options)
  {
    ...
    int offset = 12;
    if ((channel_options & laser_geometry::channel_option::Intensity) && scan_in.intensities.size() > 0)
    {
    ...
    if ((channel_options & laser_geometry::channel_option::Index))
    {
      int field_size = cloud_out.fields.size();
      cloud_out.fields.resize(field_size + 1);
    ...
    if ((channel_options & laser_geometry::channel_option::Distance))
    {
      int field_size = cloud_out.fields.size();
      cloud_out.fields.resize(field_size + 1);
    ...
    if ((channel_options & laser_geometry::channel_option::Timestamp))
    {
      int field_size = cloud_out.fields.size();
      cloud_out.fields.resize(field_size + 1);
    ...
    if ((channel_options & laser_geometry::channel_option::Viewpoint))
    {
      int field_size = cloud_out.fields.size();
                                                        ...
                                                        int channel_options)
  {
    ...
    channel_options |= laser_geometry::channel_option::Index;
    ...
    for(size_t i = 0; i < cloud_out.width; ++i)
    {
      ...
      tfScalar ratio = rotation_direction_?pt_index * ranges_norm:1.0-pt_index * ranges_norm;
      ...
      // Compute the slerp-ed rotation
      cur_transform.setRotation (tf2::slerp (quat_start, quat_end , ratio));
